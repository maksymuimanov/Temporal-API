...
public class InjectionAnnotationProcessor implements AnnotationProcessor {
    ...
    public void process(AnnotationStrategyConsumer consumer, StrategyPool strategyPool, Set<Class<?>> classes) {
        consumer.execute(MetadataLayer.FIELD_EXECUTOR, strategyPool.getAll(StrategyPoolInitializer.DEFAULT_FIELD_INJECTION), classes);
        consumer.execute(MetadataLayer.METHOD_EXECUTOR, strategyPool.getAll(StrategyPoolInitializer.DEFAULT_METHOD_INJECTION), classes);
    }
}
